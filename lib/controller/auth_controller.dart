...
class AuthController extends GetxController {
  ...
  decideRoute() {
    ...
    if (user != null) {
      FirebaseFirestore.instance
        ...
        if (isLoginAsDriver) {
          if (value.exists) {
            isDriverApproved().then((approved) {
              if (approved) {
                Get.offAll(() => CarRegistrationTemplate());
              } else {
                Get.offAll(() => VerificationPendingScreen());
              }
            });
          } else {
  ...
  Future<bool> isDriverApproved() async {
    ...
    return (userDoc.data() as Map<String, dynamic>)['isApproved'] ?? false;
  }

  void checkApprovalStatus() async {
    bool approved = await isDriverApproved();
    if (approved) {
      Get.offAll(() => const CarRegistrationTemplate());
    } else {
      Get.snackbar('Not Approved', 'Your account is still under review. Please check back later.');
    }
  }
}